...
module.exports = {
	...
	async index(request, response) {
		...
		if(newChar.length > 0) {
			return response.status(400).json('Uh! Sorry but it seems that this super epic hero or not char name its already taken! Would you be so kind to give it another go ? Think Chuck Norris + Agamemnon, like Chagamem Nonnis!')
		} 
		try {
			await db(`chars`)
					.insert({
